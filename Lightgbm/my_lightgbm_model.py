...
def load_data_with_data_loader(healthy_files, stroke_files, feature_dim=64, batch_size_processing=500,
...
def train_single_model(train_features, train_labels, val_features, val_labels,
                       params, early_stopping_rounds=20, verbose=False):
    """
    训练单个 LightGBM 模型，使用 early_stopping 并保存验证集 loss 最小的模型
    兼容新旧版本 LightGBM：
        - 新版直接使用 early_stopping_rounds 参数
        - 旧版使用回调并重新训练最佳迭代模型
    返回：训练好的模型（最佳迭代），以及验证集上的各项指标
    """
    model = LGBMClassifier(**params)
    try:
        # 尝试使用 early_stopping_rounds 参数（新版 LightGBM）
        model.fit(
        ...
        # 旧版 LightGBM，使用回调
        print("当前LightGBM版本不支持 early_stopping_rounds 参数，使用回调方式并重新训练最佳模型")
        model.fit(
            train_features, train_labels,
            eval_set=[(val_features, val_labels)],
            eval_metric='binary_logloss',
        ...
        model.fit(
            ...
            eval_set=[(val_features, val_labels)],
            eval_metric='binary_logloss',
            # verbose=verbose  # 无需早停，直接训练到 best_iter
        )
    # 预测验证集概率和类别（使用最佳模型）
...
def evaluate_model(models, X_test, y_test, use_ensemble=True):
    """原始评估函数（保持兼容）"""
    if isinstance(models, list) and use_ensemble:
        ...
        model_info = f"集成模型 ({len(models)}个)"
    else:
    ...
    metrics = compute_binary_metrics(y_test, y_pred, y_pred_proba)
    print(f"\n=== {model_info} 评估结果 (已平衡测试集) ===")
...
def plot_field_importance(importance_dict, top_n=20, save_path=None, title_suffix=""):
    ...
    plt.yticks(range(len(df)), df['feature'].values, fontsize=18)
    plt.xlabel('Importance Score', fontsize=18)
    plt.title(f'Field Importance Ranking (Top {top_n}) {title_suffix}', fontsize=20, pad=15)
...
def plot_feature_importance_ensemble(models, field_names, feature_names, top_n=20, save_path=None):
    """绘制集成模型的特征重要性图（字段级别）- 保留原接口，内部调用新函数"""
...
def test_ensemble_models(models, model_metrics_list, X_test, y_test,
    ...
    参数:
        ...
        ensemble_method: 集成方法
            ...
            - 'max_voting': 最大投票法
        selection_metric: 用于加权或记录的指标名称，可选 'accuracy','sensitivity','specificity','f1','balanced_accuracy','g_mean','auc'
    """
    n_models = len(models)
    if n_models == 0:
        print("没有模型可供集成")
    ...
    if ensemble_method == 'selection_metric_weighted':
        ...
        weights = None  # 投票法不需要权重，但在平票时会用加权概率
...
def run_single_experiment(run_id, seed):
    """执行一次完整的训练和评估流程，返回集成方法的测试指标字典和字段重要性字典"""
    # 设置随机种子
    np.random.seed(seed)
    ...
    if val_class_counts[0] != val_class_counts[1]:
        X_val, y_val = balance_data_undersample(X_val, y_val)
    if test_class_counts[0] != test_class_counts[1]:
        X_test, y_test = balance_data_undersample(X_test, y_test)

    print(f"平衡后验证集: {X_val.shape}, 分布: {np.bincount(y_val.astype(int))}")
    print(f"平衡后测试集: {X_test.shape}, 分布: {np.bincount(y_test.astype(int))}")
    ...
    # LightGBM 参数（将 random_state 改为传入的种子）
    params = {
        'objective': 'binary',
        'metric': 'auc',
        'learning_rate': 0.05,
    ...
    # 创建本次运行的输出目录
    output_dir = f'./lightgbm_easyensemble_output_run_{run_id+1}'
    os.makedirs(output_dir, exist_ok=True)

    # 保存所有模型及指标
    for i, model in enumerate(models):
        joblib.dump(model, os.path.join(output_dir, f'lightgbm_model_{i+1}.pkl'))
    # 保存指标列表
    with open(os.path.join(output_dir, 'model_metrics.json'), 'w') as f:
        serializable = []
        for m in model_metrics_list:
            serializable.append({k: float(v) if isinstance(v, (np.floating, float)) else v
    ...
    # 绘制ROC曲线（等权重平均）- 每次运行单独保存
    plot_roc_curve_ensemble(models, X_test, y_test,
    ...
    ensemble_methods = [
        ...
        'selection_metric_weighted',  # 默认使用f1
        'max_voting'
    ]
    results = {}
    for method in ensemble_methods:
        sel = 'f1' if method == 'selection_metric_weighted' else None
        metrics = test_ensemble_models(
            models, model_metrics_list, X_test, y_test,
...
def main():
    n_runs = 3
    base_seed = 42
    all_results = []          # 每次运行的 results 字典列表
    ...
    if all_field_importance_dicts:
        ...
        # 创建平均结果输出目录
        avg_output_dir = './lightgbm_easyensemble_output'
    ...
    for method in method_names:
        summary[method] = {}
        for metric in ['accuracy','precision','sensitivity','specificity','f1','auc']:
